...
def Press(): #This function is called when the button is pressed
    ...
    if (sum(Check)==13):
        ...
        nB_C,nB_V_Top,nB_V_Bot,N_barra_C,N_barra_Vt,N_barra_Vb=Modeling(Range)#Function to plot the pushover prediction
        Drawing(nB_C,nB_V_Top,nB_V_Bot,N_barra_C,N_barra_Vt,N_barra_Vb)#Function to draw structure profile, beams and columns
        
def Drawing(nB_C,nB_V_Top,nB_V_Bot,N_barra_C,N_barra_Vt,N_barra_Vb):#This function draw the structure profile, beams and columns
    #Structure profile
    ...
    Col.create_text(xC,yC, text = '#'+str(N_barra_C), font=("Arial",8))
    #Reinforcement
    recub=5
    dim=7
    ...
    Col.create_text(xB,yB, text = '#'+str(N_barra_Vt), font=("Arial",8))
    #Reinforcement
...
def Modeling(Range):#This funtion plot the pushover prediction
    ...
    nB_C,A_barras_C,nLineasAcero_C,num_barra=DimRefuerzo(B_VIG,As_C,'Columna') #Define el acero comercial y distribución necesaria en columna según cuantía
    C_ColR=(sum(nB_C)*A_barras_C[0])/(B_VIG*B_VIG)
    N_barra_C=num_barra[0]
    #Vigas
    As_V=[CUANTIA_VIG_SUP*B_VIG*(H_VIG-recub),CUANTIA_VIG_INF*B_VIG*(H_VIG-recub)]
    n_barras_V,A_barras_V,nLineasAcero_V,num_barra=DimRefuerzo(B_VIG,As_V,'Viga')  #Define el acero comercial y distribución necesaria en viga según cuantías
    ...
    N_barra_Vt=num_barra[0]
    N_barra_Vb=num_barra[1]
    ...
    return nB_C,nB_V_Top,nB_V_Bot,N_barra_C,N_barra_Vt,N_barra_Vb
